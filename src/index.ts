...
class GodotServer {
  ...
  constructor(config?: GodotServerConfig) {
    ...
    this.docsManager = DocsManager.getInstance();
    ...
    try {
      const configPath = resolve(process.cwd(), 'godot-mcp.config.json');
      if (existsSync(configPath)) {
        this.logDebug(`Loading config from ${configPath}`);
        const fileConfig = JSON.parse(readFileSync(configPath, 'utf8'));
        if (fileConfig.godotPath) {
          this.godotPath = normalize(fileConfig.godotPath);
          this.logDebug(`Loaded Godot path from config file: ${this.godotPath}`);
        }
        if (fileConfig.debugMode !== undefined) debugMode = fileConfig.debugMode;
        if (fileConfig.godotDebugMode !== undefined) godotDebugMode = fileConfig.godotDebugMode;
        if (fileConfig.strictPathValidation !== undefined) this.strictPathValidation = fileConfig.strictPathValidation;
      }
    } catch (error) {
      console.warn(`[SERVER] Failed to load config file: ${error}`);
    }
  ...
  private async isValidGodotPath(path: string): Promise<boolean> {
    ...
    try {
      ...
      console.error(`[SERVER] Executing command: ${command}`);
      
      const { stdout, stderr } = await execAsync(command);
      console.error(`[SERVER] Command success. Stdout: ${stdout.trim()}`);
      ...
      this.logDebug(`Invalid Godot path: ${path}, error: ${error}`);
      console.error(`[SERVER] Invalid Godot path: ${path}`);
      console.error(`[SERVER] Error details: ${error.message}`);
      if (error.stdout) console.error(`[SERVER] Stdout: ${error.stdout}`);
      if (error.stderr) console.error(`[SERVER] Stderr: ${error.stderr}`);
  ...
  private async detectGodotPath() {
    ...
    if (osPlatform === 'darwin') {
      ...
      possiblePaths.push(
        'C:\\Users\\clare\\Downloads\\Godot_v4.4.1-stable_win64.exe\\Godot_v4.4.1-stable_win64.exe',
        'C:\\Users\\clare\\Downloads\\Godot_v4.4.1-stable_win64.exe',
    ...
    if (this.strictPathValidation) {
      ...
      // Fallback: Check if we should use a default path or fail
      if (process.env.GODOT_PATH) {
          // If GODOT_PATH was provided but invalid (checked earlier), we shouldn't guess
          this.logDebug('GODOT_PATH was provided but invalid. Not using fallbacks.');
      } else {
         this.logDebug('No GODOT_PATH provided and auto-detection failed.');
      }
      
      // We do NOT set a hardcoded default path anymore to avoid confusion.
      // The user must explicitly set GODOT_PATH or ensure Godot is in standard locations.
      console.warn(`[SERVER] Could not find Godot. Please set the GODOT_PATH environment variable.`);
    }
  }
  ...
  /**
   * Helper to get project name from project.godot
   */
  private getProjectName(projectPath: string): string | null {
    try {
  ...
  private findGodotProjects(directory: string, recursive: boolean): Array<{ path: string; name: string }> {
    ...
    try {
      ...
      const projectFile = join(absoluteDirectory, 'project.godot');
      if (existsSync(projectFile)) {
        projects.push({
          path: absoluteDirectory,
          name: this.getProjectName(projectFile) || basename(absoluteDirectory),
        });
      }
      ...
      if (!recursive) {
        ...
        for (const entry of entries) {
          if (entry.isDirectory()) {
            const subdir = join(absoluteDirectory, entry.name);
            const projectFile = join(subdir, 'project.godot');
            if (existsSync(projectFile)) {
              projects.push({
                path: subdir,
                name: this.getProjectName(projectFile) || entry.name,
              });
            }
          }
        }
  ...
  private setupToolHandlers() {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              nodePath: {
                type: 'string',
                description: 'Path to the node to edit (e.g., "root/Player/Sprite2D")',
              },
              properties: {
                type: 'object',
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              nodePath: {
                type: 'string',
                description: 'Path to the node to remove (e.g., "root/Player/Sprite2D")',
              },
            },
        ...
        },
        {
          ...
          description: 'Get a tree of available Godot documentation topics',
          inputSchema: {
            type: 'object',
            properties: {},
            required: [],
        ...
        },
        {
          ...
          description: 'Get the content of a specific documentation file',
          inputSchema: {
            type: 'object',
            properties: {
              path: {
                type: 'string',
                description: 'Path to the documentation file (from get_documentation_tree)',
              },
            },
            required: ['path'],
          },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      switch (request.params.name) {
        ...
        case 'edit_node':
          return await this.handleEditNode(request.params.arguments);
        case 'remove_node':
  ...
  private async handleGetDebugOutput() {
    const process = this.activeProcess || this.lastProcess;
    if (!process) {
      return this.createErrorResponse(
        'No active or recent Godot process.',
        [
          ...
          text: JSON.stringify(
            {
              ...
              status: this.activeProcess ? 'running' : 'stopped'
            },
            null,
            2
          ),
  ...
  private async handleCreateScene(args: any) {
    ...
    try {
      ...
      if (response && response.status === 'success') {
         ...
         return this.createErrorResponse(`Failed to create scene: ${response.message}`);
      }
  ...
  private async handleAddNode(args: any) {
    ...
    try {
      ...
      if (response && response.status === 'success') {
         ...
         return this.createErrorResponse(`Failed to add node: ${response.message}`);
      }
  ...
  private async handleSaveScene(args: any) {
    ...
    try {
      ...
      if (response && response.status === 'success') {
         ...
         return this.createErrorResponse(`Failed to save scene: ${response.message}`);
      }
  ...
  private async handleEditNode(args: any) {
    ...
    try {
      ...
      // Execute the operation
      const { stdout, stderr } = await this.executeOperation('edit_node', params, args.projectPath);
      ...
      if (stderr && stderr.includes('Failed to')) {
        return this.createErrorResponse(
          `Failed to edit node: ${stderr}`,
          [
            'Check if the node path is correct',
            'Ensure the properties are valid for the node type',
            'Verify the scene file is valid',
          ]
        );
      }
      ...
      if (response && response.status === 'success') {
         return {
            content: [{ type: 'text', text: `Node '${args.nodePath}' edited successfully in '${args.scenePath}'.` }]
         };
      } else if (response && response.status === 'error') {
         return this.createErrorResponse(`Failed to edit node: ${response.message}`);
      }
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Node '${args.nodePath}' edited successfully in '${args.scenePath}'.\n\nOutput: ${stdout}`,
          },
        ],
      };
    } catch (error: any) {
      return this.createErrorResponse(
        `Failed to edit node: ${error?.message || 'Unknown error'}`,
        [
          'Ensure Godot is installed correctly',
          'Check if the GODOT_PATH environment variable is set correctly',
          'Verify the project path is accessible',
  ...
  private async handleRemoveNode(args: any) {
    ...
    try {
      ...
      if (response && response.status === 'success') {
         return {
            content: [{ type: 'text', text: `Node '${args.nodePath}' removed successfully from '${args.scenePath}'.` }]
         };
      } else if (response && response.status === 'error') {
         return this.createErrorResponse(`Failed to remove node: ${response.message}`);
      }
  ...
  /**
   * Handle the get_documentation_tree tool
   */
  private async handleGetDocumentationTree() {
    try {
      this.logDebug('Getting documentation tree');
      const tree = await this.docsManager.getDocumentationTree();
      ...
      return this.createErrorResponse(
        `Failed to get documentation tree: ${error?.message || 'Unknown error'}`,
        [
          'Check internet connection if documentation needs to be downloaded',
          'Ensure the docs folder exists or is writable',
        ]
      );
    }
  }
  ...
  /**
   * Handle the get_documentation_file tool
   */
  private async handleGetDocumentationFile(args: any) {
    if (!args.path) {
      return this.createErrorResponse(
        'Missing required parameter: path',
        ['Provide the path to the documentation file']
      );
    }

    try {
      this.logDebug(`Getting documentation file: ${args.path}`);
      const content = await this.docsManager.getFileContent(args.path);
      ...
      return this.createErrorResponse(
        `Failed to get documentation file: ${error?.message || 'Unknown error'}`,
        [
          'Check if the file path is correct (use get_documentation_tree to find paths)',
          'Ensure the docs folder exists and is accessible',
        ]
      );
    }
  }
