...
# Print a JSON response for the MCP server to parse
func print_json_response(status, message, data = {}):
    var response = {
        "status": status,
        "message": message
    }
...
func create_scene(params):
    ...
    if result == OK:
        ...
        if save_error == OK:
            ...
            var abs_path = ProjectSettings.globalize_path(full_scene_path)
            var verified = FileAccess.file_exists(full_scene_path) or FileAccess.file_exists(abs_path)
            
            if verified:
                if debug_mode:
                    print("Scene created and verified successfully at: " + params.scene_path)
                print_json_response("success", "Scene created successfully", {"path": params.scene_path})
            else:
                printerr("Failed to verify file creation: " + params.scene_path)
                print_json_response("error", "Failed to verify file creation", {"path": params.scene_path})
                quit(1)
        else:
            ...
            print_json_response("error", error_message, {})
            quit(1)
    else:
...
func save_scene(params):
    ...
    if result == OK:
        ...
        if error == OK:
            ...
            var verified = FileAccess.file_exists(save_path) or FileAccess.file_exists(abs_path)
            
            if verified:
                if debug_mode:
                    print("File exists check after save: true")
                    print("Scene saved successfully to: " + save_path)
                print_json_response("success", "Scene saved successfully", {"path": save_path})
            else:
                printerr("File reported as saved but does not exist at: " + save_path)
                print_json_response("error", "File reported as saved but does not exist", {"path": save_path})
                quit(1)
        else:
            printerr("Failed to save scene: " + str(error))
            print_json_response("error", "Failed to save scene", {"error_code": error})
            quit(1)
    else:
        printerr("Failed to pack scene: " + str(result))
        print_json_response("error", "Failed to pack scene", {"error_code": result})
        quit(1)
...
func edit_node(params):
    ...
    if result == OK:
        ...
        if save_error == OK:
            ...
            print_json_response("success", "Node properties updated successfully", {"node_path": node_path})
        else:
            printerr("Failed to save scene: " + str(save_error))
            print_json_response("error", "Failed to save scene", {"error_code": save_error})
            quit(1)
    else:
        printerr("Failed to pack scene: " + str(result))
        print_json_response("error", "Failed to pack scene", {"error_code": result})
...
func remove_node(params):
    ...
    if result == OK:
        ...
        if save_error == OK:
            ...
            print_json_response("success", "Node removed successfully", {"node_path": node_path})
        else:
            printerr("Failed to save scene: " + str(save_error))
            print_json_response("error", "Failed to save scene", {"error_code": save_error})
            quit(1)
